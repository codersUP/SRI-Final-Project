...
El objetivo principal del proyecto es la implementación de dos modelos de Sistemas de Recuperación de Información y a su vez aplicarlos a diferentes escenarios para lograr una comparación entre los mismos. Los modelos escogidos son el Booleano y el Vectorial.

\section{Herramientas}

Es necesario pre-procesar los documentos para obtener las matrices de \'indices (index) and \'indices invertidos (inverted index). En el caso de este proyecto en particular se sustituye la matriz por una serie de diccionarios de Python anidados que en final cumplen el mismo objetivo.\\

(Aquí poner algo que tenga q ver con los .json si quieres)

\subsection{parse\_documents.py}

\begin{verbatim}
  def create_index_and_inverse_index(documents, verbose=False)
\end{verbatim}

Este método recibe el conjunto de documentos (dataset) y su nombre de forma explícita explica que devuelve el mismo. El resultado es una tupla (index,inverse\_index).
...
Se analiza en este método cada uno de los documentos que son procesados a través de los siguientes pasos:

\begin{verbatim}
  def get_importants_words(text: str):
\end{verbatim}

Esta función hace uso de la biblioteca spacy para tokenizar el documento y después eliminar aquellos tokens que no son de importancia para el procesamiento: signos de puntuación, de monedas, dígitos, espacios, stopwords.

Despúes de este filtrado inicial se van construyendo los diccionarios index e inverse\_index con apoyo de los métodos:

\begin{verbatim}
  create_index(index, document, word)
\end{verbatim}

Recibe el diccionario que va a editar, el documento y palabra en específico que se está analizando en el instante. Teniendo como llave del diccionario al documento; crea/edita los valores: el tamaño en palabras, los términos y su frecuencia en el documento. 

\begin{verbatim}
  check_max_frequency(index, document, word)
\end{verbatim}

Actualiza si es necesario, la mayor frecuencia del documento para su uso posterior en el cálculo de {\it tf}.

\begin{verbatim}
  create_inverse_index(inverse_index, document, word)
\end{verbatim}

Guarda/edita el valor de la llave {\it word} en el diccionario {\it inverse\_index}, de manera que contenta todos los documentos en los que dicha palabra ha aparecido hasta el momento y con que frecuencia.

\begin{verbatim}
  def create_tf(index, document):
\end{verbatim}

Cuando se  analizan todos los términos de un documento, entonces con los datos recopilados como base se calcula el tf de cada uno de los términos y se añade como llave al diccionario de cada término.\\

Al finalizar este proceso con cada uno de los documentos se calculan los {\it idf} y por último {\it tf*idf}:

\begin{verbatim}
  create_tf_idf(index, inverse_index)
\end{verbatim}

Esos valores tambíen son almacenados en el diccionario asociado a cada término en {\it index}.\\

Este pre-procesamiento permite tener todos los valores necesarios para el cálculo de pesos y por consecuencia de similitud en el modelo vectorial, así como los conjuntos necesarios para el análisis de pertenencia en el booleano.

\section{Modelo Booleano}

El problema principal asociado a este modelo está dado por el análisis de la query. Haciendo uso de la biblioteca ply se implementó un lexer y un parser para facilitar la evaluación de la misma contra los datos preprocesados.

\subsection{query\_lexer.py}
...
El lexer, divide en tokens la query y desecha aquello que no se considera necesario. Al mismo tiempo apoyándose en la biblioteca spacy, le aplica lemmatise a cada término, de la misma forma que se le realizó a los documentos.

\subsection{query\_parser.py}

Ya con los tokens definidos, se implementa el parser con yacc partiendo de la gramática:
...
El parser evalua cada {\it nodo} del árbol que construye el parser como un diccionario que contiene información valiosa para la resolución posterior del conjunto que cumple con las premisas de la query.

\subsection{boolean\_model.py}

\begin{verbatim}
  def get_documents(query, inverse_index, documents):
\end{verbatim}

Este método es el encargado de  devolver el conjunto de documentos que cumplen con las restricciones de la query. Parsea la query y retornael resultado del llamado a:

\begin{verbatim}
def calculate_set(boolean_expression_dict, inverse_index, documents):
\end{verbatim}

Recibe como primer parámetro el resultado de parsear la query. Función que realiza un procesamiento análogo a un visitor, que a través de la distinción de los diferentes {\it nodos}; los evalúa conformando conjuntos que resultan de operaciones de intersecciones y uniones sobre los mismos, hasta llegar al definitivo. Este último contiene todos los documentos que cumplen con las condiciones presentadas en forma de query por el usuario.

\section{Modelo vectorial}

\subsection{parse\_query.py}

\begin{verbatim}
  def create_index_query(query, a, document_inverse_index):
\end{verbatim}

Función que es análoga de cierta manera al método que crea {\it index} e {\it inverse\_index} cuando se parsea el documento. Se define con los parámetros {\it a} y {\it document\_inverse\_index}, el primero es un coeficiente de amortización en el cálculo de {\it tf} de la query y el segundo es utilizado para calcular el {\it idf} de los términos.

\subsection{vectorial\_model.py}

Ya con los {\it tf*idf} de los documentos y la query calculados en sus matrices de {\it index}, solo queda calcular el ranking de los documentos con dichos datos:

\begin{verbatim}
  def calculate_rank(query_index, index, rank_N):
\end{verbatim}

Recibe además el parámetro {\it rank\_N} que define la cantidad de documentos que se van a mostrar como resultado al usuario. Está función ordena de forma descendente los valores de similitud de cada documento con la query en cuestión. Este valor se computa en:

\begin{verbatim}
  def rank_query_document(query_index, document_features):
\end{verbatim}

Función que calcula la similitud entre la query y el documento. Es el resultado de la aplicación de la regla del coseno donde se realiza el producto escalar de los vectores de peso dividido por la múltiplicación de la norma de los mismos.

\subsection{expand\_query.py}

La expansión de la query se hace con el objetivo de sugerir al usuario una query que contenga términos más similares a los contenidos en los documentos y lograr una posible mejora del proceso de recuperación de información.

\begin{verbatim}
  def expand_query(query_index, inverse_index):
\end{verbatim}

Crea un diccionario {\it expanded}  donde se agregan como llaves aquellos términos cuyo peso fue 0. Esto significa que no aportaron nada en el cálculo de similitud. A cada llave se le asocia un término similar, que después se sustituirá en la query.

\begin{verbatim}
  def replace_expand_in_query(query, expanded):
\end{verbatim}

Todas las llaves del diccionario que estén contenidas en los términos de la query, se sustituye por el valor que resulta de indexar la llave en el diccionario. 
